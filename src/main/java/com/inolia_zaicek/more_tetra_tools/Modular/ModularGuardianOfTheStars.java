...
public class ModularGuardianOfTheStars extends ItemModularHandheld { // 声明一个名为ModularNecklace的公共类，它继承自ModularItem并实现ICurio接口。
    //部件类型/槽位——[slot]
    public final static String modular_guardian_of_the_stars_blade = "modular_guardian_of_the_stars/blade";
    public final static String modular_guardian_of_the_stars_handle = "modular_guardian_of_the_stars/handle";
    public final static String modular_guardian_of_the_stars_guard = "modular_guardian_of_the_stars/guard";
    public final static String modular_guardian_of_the_stars_counterweight = "modular_guardian_of_the_stars/counterweight";
    public final static String modular_guardian_of_the_stars_socket = "modular_guardian_of_the_stars/socket";
    // --- 物品标识符 ---
    /*** 模块化项链在模组内的唯一标识符。* 这个标识符用于在NBT数据、配方和其他模组交互中引用该物品。*/
    public final static String identifier = "modular_guardian_of_the_stars";
    ...
    private static final GuiModuleOffsets minorOffsets = new GuiModuleOffsets(
            ...
            -12, -3);
    // --- 物品实例 ---
    /*** 使用ObjectHolder注解，使得Forge在加载时能够自动获取已注册的“tetra:modular_guardian_of_the_stars”物品实例。
     * 这允许其他模组或代码直接通过`ModularNecklace.instance`来访问这个物品，而无需手动注册和查找。
     * `registryName = "item"` 指定了要查找的注册表类型是物品。
     * `value = "tetra:modular_guardian_of_the_stars"` 指定了该物品在物品注册表中的名称。
     * 注意：此字段在编译时和运行时可能存在依赖关系，确保“tetra:modular_guardian_of_the_stars”已正确注册。*/
    //modular_guardian_of_the_stars算是类似“单头工具”、“双头工具”的物品必备ID
    @ObjectHolder(
            registryName = "item",
            value = "tetra:modular_guardian_of_the_stars"
    )
    public static ModularLaPlumaScythe instance;

    /*** ModularNecklace 类的构造函数。* 这里初始化了该模块化物品的基本属性。*/
    public ModularGuardianOfTheStars() {
        ...
        canHone = true;
        //设置主要部件有什么
        majorModuleKeys = new String[]{modular_guardian_of_the_stars_blade,  modular_guardian_of_the_stars_handle};
        //设置次要部件有什么
        minorModuleKeys = new String[]{modular_guardian_of_the_stars_guard, modular_guardian_of_the_stars_socket,modular_guardian_of_the_stars_counterweight};
        // 定义该项链所必需的模块（Required Modules）。游戏会确保这些模块至少存在一个，否则物品可能无法正常工作或显示。
        requiredModules = new String[]{modular_guardian_of_the_stars_blade, modular_guardian_of_the_stars_handle};
        //可修复
    ...
    public void commonInit(PacketHandler packetHandler) {
        DataManager.instance.synergyData.onReload(() -> synergies =
                DataManager.instance.synergyData.getOrdered("modular_guardian_of_the_stars/"));
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand pUsedHand) {
        //时间【一天24000】0——6点、6000——12点、12000————18点、11616.6————日落、13800————完全日落、22200————日出
        //清晨(451--6000)
        //正午(6001--12000)
        //黄昏(12001--13800)
        //黑夜(13801--22200)
        //黎明(22201-24000/0-450)
        if (!level.isClientSide() && pUsedHand == InteractionHand.MAIN_HAND) {
            ...
            &&!player.getCooldowns().isOnCooldown(player.getMainHandItem().getItem())) {
                level.getDayTime();
                //获取词条等级
                int effectLevel = item.getEffectLevel(player.getMainHandItem(), astralSwordEffect);
                if (effectLevel > 0) {
                    long gameTime = level.getDayTime();
                    player.removeEffect(MTTEffectsRegister.AstralSwordMorning.get());
                    player.removeEffect(MTTEffectsRegister.AstralSwordAfternoon.get());
                    player.removeEffect(MTTEffectsRegister.AstralSwordDusk.get());
                    player.removeEffect(MTTEffectsRegister.AstralSwordNight.get());
                    player.removeEffect(MTTEffectsRegister.AstralSwordDawn.get());
                    if(gameTime>=451&&gameTime<=6000){
                        player.addEffect(new MobEffectInstance(MTTEffectsRegister.AstralSwordMorning.get(), 20 * 15, 0));
                    }
                    if(gameTime>=6001&&gameTime<=12000) {
                        player.addEffect(new MobEffectInstance(MTTEffectsRegister.AstralSwordAfternoon.get(), 20 * 15, 0));
                    }
                    if(gameTime>=12001&&gameTime<=13800) {
                        player.addEffect(new MobEffectInstance(MTTEffectsRegister.AstralSwordDusk.get(), 20 * 15, 0));
                    }
                    if(gameTime>=13801&&gameTime<=22200) {
                        player.addEffect(new MobEffectInstance(MTTEffectsRegister.AstralSwordNight.get(), 20 * 15, 0));
                    }
                    if(gameTime>=22201&&gameTime<=24000) {
                        player.addEffect(new MobEffectInstance(MTTEffectsRegister.AstralSwordDawn.get(), 20 * 15, 0));
                    }
                    if(gameTime>=0&&gameTime<=450) {
                        player.addEffect(new MobEffectInstance(MTTEffectsRegister.AstralSwordDawn.get(), 20 * 15, 0));
                    }
                }
                player.getCooldowns().addCooldown(this, 20 * 10);//设置冷却时间
            }
            ...
            if (player.getOffhandItem().getItem() instanceof IModularItem item
                ...
                //获取词条等级
                int effectLevel = item.getEffectLevel(player.getOffhandItem(), divineAvatarEffect);
                if (effectLevel > 0) {
                    long gameTime = level.getDayTime();
                    player.removeEffect(MTTEffectsRegister.AstralSwordMorning.get());
                    player.removeEffect(MTTEffectsRegister.AstralSwordAfternoon.get());
