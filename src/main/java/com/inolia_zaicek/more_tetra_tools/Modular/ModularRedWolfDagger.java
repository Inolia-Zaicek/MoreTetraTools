...
import static com.inolia_zaicek.more_tetra_tools.Effect.Clent.MTTEffectGuiStats.ExecutionModeEffect;

@SuppressWarnings({"all", "removal"})
public class ModularRedWolfDagger extends ItemModularHandheld { // 声明一个名为ModularNecklace的公共类，它继承自ModularItem并实现ICurio接口。
    //部件类型/槽位——[slot]
    public final static String modular_red_wolf_dagger_blade = "modular_red_wolf_dagger/blade";
    public final static String modular_red_wolf_dagger_handle = "modular_red_wolf_dagger/handle";
    public final static String modular_red_wolf_dagger_guard = "modular_red_wolf_dagger/guard";
    public final static String modular_red_wolf_dagger_counterweight = "modular_red_wolf_dagger/counterweight";
    public final static String modular_red_wolf_dagger_socket = "modular_red_wolf_dagger/socket";
    // --- 物品标识符 ---
    /*** 模块化项链在模组内的唯一标识符。* 这个标识符用于在NBT数据、配方和其他模组交互中引用该物品。*/
    public final static String identifier = "modular_red_wolf_dagger";
    ...
    private static final GuiModuleOffsets minorOffsets = new GuiModuleOffsets(
            ...
            -12, -3);
    // --- 物品实例 ---
    /*** 使用ObjectHolder注解，使得Forge在加载时能够自动获取已注册的“tetra:modular_red_wolf_dagger”物品实例。
     * 这允许其他模组或代码直接通过`ModularNecklace.instance`来访问这个物品，而无需手动注册和查找。
     * `registryName = "item"` 指定了要查找的注册表类型是物品。
     * `value = "tetra:modular_red_wolf_dagger"` 指定了该物品在物品注册表中的名称。
     * 注意：此字段在编译时和运行时可能存在依赖关系，确保“tetra:modular_red_wolf_dagger”已正确注册。*/
    //modular_red_wolf_dagger算是类似“单头工具”、“双头工具”的物品必备ID
    @ObjectHolder(
            registryName = "item",
            value = "tetra:modular_red_wolf_dagger"
    )
    public static ModularLaPlumaScythe instance;

    /*** ModularNecklace 类的构造函数。* 这里初始化了该模块化物品的基本属性。*/
    public ModularRedWolfDagger() {
        ...
        canHone = true;
        //设置主要部件有什么
        majorModuleKeys = new String[]{modular_red_wolf_dagger_blade,  modular_red_wolf_dagger_handle};
        //设置次要部件有什么
        minorModuleKeys = new String[]{modular_red_wolf_dagger_guard, modular_red_wolf_dagger_socket,modular_red_wolf_dagger_counterweight};
        // 定义该项链所必需的模块（Required Modules）。游戏会确保这些模块至少存在一个，否则物品可能无法正常工作或显示。
        requiredModules = new String[]{modular_red_wolf_dagger_blade, modular_red_wolf_dagger_handle};
        //可修复
    ...
    public void commonInit(PacketHandler packetHandler) {
        DataManager.instance.synergyData.onReload(() -> synergies =
                DataManager.instance.synergyData.getOrdered("modular_red_wolf_dagger/"));
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand pUsedHand) {
        if (!level.isClientSide() && pUsedHand == InteractionHand.MAIN_HAND) {
            ...
            &&!player.getCooldowns().isOnCooldown(player.getMainHandItem().getItem())) {
                //获取词条等级
                int effectLevel = item.getEffectLevel(player.getMainHandItem(), ExecutionModeEffect);
                if (effectLevel > 0) {
                    player.addEffect(new MobEffectInstance(MobEffects.INVISIBILITY, 20 * 20, 0));
                    //有铁魔法的情况下额外附加真实隐身
                    if (ModList.get().isLoaded("irons_spellbooks")) {
                        player.addEffect(new MobEffectInstance(Objects.requireNonNull(
                                ForgeRegistries.MOB_EFFECTS.getValue(new ResourceLocation("irons_spellbooks", "true_invisibility"))),
                                20 * 20, 0));
                    }
                }
                player.getCooldowns().addCooldown(this, 20 * 4);//设置冷却时间
            }
            ...
            if (player.getOffhandItem().getItem() instanceof IModularItem item
                ...
                //获取词条等级
                int effectLevel = item.getEffectLevel(player.getOffhandItem(), ExecutionModeEffect);
                if (effectLevel > 0) {
                    player.addEffect(new MobEffectInstance(MobEffects.INVISIBILITY, 20 * 20, 0));
                    //有铁魔法的情况下额外附加真实隐身
                    if (ModList.get().isLoaded("irons_spellbooks")) {
