...
public class ModularMasterOfLight extends ItemModularHandheld { // 声明一个名为ModularNecklace的公共类，它继承自ModularItem并实现ICurio接口。
    //部件类型/槽位——[slot]
    public final static String modular_master_of_light_blade = "modular_master_of_light/blade";
    public final static String modular_master_of_light_handle = "modular_master_of_light/handle";
    public final static String modular_master_of_light_guard = "modular_master_of_light/guard";
    public final static String modular_master_of_light_counterweight = "modular_master_of_light/counterweight";
    public final static String modular_master_of_light_socket = "modular_master_of_light/socket";
    // --- 物品标识符 ---
    /*** 模块化项链在模组内的唯一标识符。* 这个标识符用于在NBT数据、配方和其他模组交互中引用该物品。*/
    public final static String identifier = "modular_master_of_light";
    ...
    private static final GuiModuleOffsets minorOffsets = new GuiModuleOffsets(
            ...
            -12, -3);
    // --- 物品实例 ---
    /*** 使用ObjectHolder注解，使得Forge在加载时能够自动获取已注册的“tetra:modular_master_of_light”物品实例。
     * 这允许其他模组或代码直接通过`ModularNecklace.instance`来访问这个物品，而无需手动注册和查找。
     * `registryName = "item"` 指定了要查找的注册表类型是物品。
     * `value = "tetra:modular_master_of_light"` 指定了该物品在物品注册表中的名称。
     * 注意：此字段在编译时和运行时可能存在依赖关系，确保“tetra:modular_master_of_light”已正确注册。*/
    //modular_master_of_light算是类似“单头工具”、“双头工具”的物品必备ID
    @ObjectHolder(
            registryName = "item",
            value = "tetra:modular_master_of_light"
    )
    public static ModularLaPlumaScythe instance;

    /*** ModularNecklace 类的构造函数。* 这里初始化了该模块化物品的基本属性。*/
    public ModularMasterOfLight() {
        ...
        canHone = true;
        //设置主要部件有什么
        majorModuleKeys = new String[]{modular_master_of_light_blade,  modular_master_of_light_handle};
        //设置次要部件有什么
        minorModuleKeys = new String[]{modular_master_of_light_guard, modular_master_of_light_socket,modular_master_of_light_counterweight};
        // 定义该项链所必需的模块（Required Modules）。游戏会确保这些模块至少存在一个，否则物品可能无法正常工作或显示。
        requiredModules = new String[]{modular_master_of_light_blade, modular_master_of_light_handle};
        //可修复
    ...
    public void commonInit(PacketHandler packetHandler) {
        DataManager.instance.synergyData.onReload(() -> synergies =
                DataManager.instance.synergyData.getOrdered("modular_master_of_light/"));
    }
