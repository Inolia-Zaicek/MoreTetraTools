    ...
    public static void hurt(LivingHurtEvent event) {
        ...
        if (event.getSource().getEntity() instanceof Player player && event.getEntity() instanceof LivingEntity) {
            ...
            if (mainHandItem.getItem() instanceof IModularItem item) {
                effectLevel += item.getEffectLevel(mainHandItem, mosterHunterEffect);
            }
            if (offhandItem.getItem() instanceof IModularItem item) {
                effectLevel += item.getEffectLevel(offhandItem, mosterHunterEffect);
            }
        ...
        else if (event.getSource().getDirectEntity() instanceof Player player && event.getEntity() instanceof LivingEntity) {
            var mob = event.getEntity();
            ItemStack mainHandItem = player.getMainHandItem();
            ItemStack offhandItem = player.getOffhandItem();
            float effectLevel = 0;
